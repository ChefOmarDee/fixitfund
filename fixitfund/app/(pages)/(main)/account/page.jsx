    ...
    <div className="min-h-screen bg-purple-100">
      ...
      <main className="container mx-auto mt-8 px-4">
        <div className="bg-white shadow-lg rounded-lg max-w-2xl mx-auto p-6">
          ...
          <div className="my-6">
            <label className="block mb-2 font-medium text-gray-700">Filter Projects:</label>
            <select value={filter} onChange={handleFilterChange} className="p-2 border border-gray-300 rounded-md">
          ...
          <div className="my-6">
            ...
            {filteredProjects.length > 0 ? (
              filteredProjects.map((project) => (
                <div key={project.projectId} className="border p-4 rounded-lg shadow mb-4">
                  <h3 className="font-semibold">{project.title}</h3>
                  <p>{project.desc}</p>
                  <p>Status: {project.status}</p>
                  <p>Cost: {parseFloat(project.cost.$numberDecimal || 0)}</p>
                  <p>Donated: {parseFloat(project.donated.$numberDecimal || 0)}</p>
                </div>
              ))
            ) : (
