...
import { DollarSign, FileText, MapPin, Tag, User, Info } from "lucide-react";
...
const ClaimProject = ({ params }) => {
		...
		const fetchProjectDetails = async () => {
			try {
				const response = await fetch(
					`/api/getprojectdetails?projectID=${params.projectID}`
				);
	...
	const handleSubmit = async (e) => {
		...
		// Check if the donationAmount is a positive number
		if (isNaN(donationValue) || donationValue <= 0) {
			setMessage("Please enter a positive donation amount.");
			setIsLoading(false);
		...
		try {
			...
			if (response.ok) {
				...
				// Redirect to the home page after successful claim
				setTimeout(() => {
				...
				{projectDetails ? (
					<div className="mb-6 p-4 bg-gray-100 rounded-md">
						...
						<p className="flex items-center mb-2">
							...
							{projectDetails.Title || "No title"}
						</p>
						<p className="flex items-center mb-2">
							<Info className="inline-block w-5 h-5 mr-2 text-black" />
							<span className="font-medium">Description:</span>{" "}
							{projectDetails.Desc || "No description"}
						</p>
						<p className="flex items-center mb-2">
							<MapPin className="inline-block w-5 h-5 mr-2 text-black" />
							<span className="font-medium">Latitude:</span>{" "}
							{projectDetails.Lat || "No location"}
						</p>
						<p className="flex items-center mb-2 text-black">
							<MapPin className="inline-block w-5 h-5 mr-2 text-black" />
							<span className="font-medium">Longitude:</span>{" "}
							{projectDetails.Long || "No location"}
						</p>
						<p className="flex items-center mb-2">
							<Tag className="inline-block w-5 h-5 mr-2 text-black" />
							<span className="font-medium">Tag:</span>{" "}
							{projectDetails.Tag || "No tag"}
						</p>

						{projectDetails.Status !== "open" && (
							<p className="text-red-500 font-medium mt-2">
								This project has already been taken or is not available for
								claiming.
							</p>
						)}
					</div>
				...
				<form onSubmit={handleSubmit} className="space-y-6">
					<div>
						...
						<input
							...
							placeholder="Enter needed donation amount"
